...
function renderAssetCard(asset) {
  ...
  return `<article class="asset-card">
    <h3>${asset.name}</h3>
    <div class="ticker">${asset.symbol} · ${asset.currency || ''} · Stand ${asset.last_date || '–'}</div>
    <div class="price"><strong>${fmtNumber(asset.last_close, asset.price_decimals ?? 2)}</strong><span>${asset.currency || ''}</span></div>
    <div class="change ${dailyCls}">${fmtPct(asset.change_pct)} heute</div>
...
function renderTable(assets) {
  tableBody.innerHTML = assets.map(a => {
    ...
    return `<tr>
      <td><strong>${a.name}</strong><br><span class="asset-meta">${a.symbol}</span></td>
...
async function loadDashboard() {
  ...
  try {
    ...
    document.querySelector('#data-source').textContent = `Datenquelle: ${data.source || 'Stooq'}`;
    ...
    assetGrid.innerHTML = `<div class="error">Daten konnten nicht geladen werden: ${error.message}</div>`;
  }
}
